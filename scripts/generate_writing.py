...
def render_engagement_section(post: dict, canonical: str) -> str:
    ...
    subscribe_source = f"article: {post['route']}"
    ...
    return f"""      <section class="engagement-section quiet-response-section" id="responses" aria-labelledby="response-title">
        <div class="quiet-response-panel will-reveal">
          <div class="quiet-response-intro">
            ...
            <p>If this piece met you where you are, there are two quiet ways to continue. You can receive the next reflection by email, or you can send a private note in plain words. No public comment section. No performance. Just a small response if something here stayed with you.</p>
          </div>

          <div class="quiet-response-grid">
            <article class="quiet-response-card">
              <h3>Get the next reflection by email</h3>
              <p>Quiet words for the days that feel heavier than expected.</p>
              <form class="signup-form subscribe-form quiet-subscribe-form" name="subscribe" method="POST" action="{BUTTONDOWN_SUBSCRIBE_ACTION}" data-subscribe-form data-analytics-source="{escape_attr(subscribe_source)}">
                <input type="hidden" value="1" name="embed">
                <input type="hidden" name="metadata__source" value="{escape_attr(subscribe_source)}">
                <div class="form-field quiet-email-field">
                  <label class="field-label" for="subscribe-email-{route}">Email address</label>
                  <input id="subscribe-email-{route}" type="email" name="email" placeholder="Email address" autocomplete="email" required>
                </div>
                <button class="button button-primary" type="submit">Subscribe</button>
              </form>
              <p class="form-note" data-form-status aria-live="polite">Please check your inbox to confirm.</p>
            </article>

            <article class="quiet-response-card">
              <h3>Share this reflection</h3>
              <p>Send it to someone who may need the words today.</p>
              <button class="button button-secondary quiet-copy-button" type="button" data-copy-link="{escape_attr(canonical)}" data-share-platform="copy_link">
                <span data-copy-text>Copy article link</span>
              </button>
            </article>
